...
router.post('/:eventId', authenticate, rsvpLimiter, async (req, res) => {
  ...
  try {
    ...
    console.log(`[Email Debug] Attempting to send RSVP confirmation for User: "${userData.name}" to Event: "${eventData.title}"`);

    try {
      await sendRsvpConfirmationEmail(userData, eventData);
      console.log(`[Email Debug] Email sent successfully for User: "${userData.name}"`);
    } catch (err) {
