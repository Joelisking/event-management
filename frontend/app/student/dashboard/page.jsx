...
export default function StudentDashboardPage() {
  ...
  const upcomingRsvps = rsvps
    .filter(
      (rsvp) =>
        getEventStatus(rsvp.event.startDate, rsvp.event.endDate) ===
        'upcoming'
    )
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-cream-50">
      {/* Background Pattern */}
      <div className="fixed inset-0 bg-[linear-gradient(to_right,#e5e5e5_1px,transparent_1px),linear-gradient(to_bottom,#e5e5e5_1px,transparent_1px)] bg-[size:4rem_4rem] opacity-20 pointer-events-none" />
        ...
        <div className="mb-12">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-black flex items-center gap-2">
              <span className="w-2 h-8 bg-pfw-gold rounded-full inline-block"></span>
            ...
            <Link href="/student/my-events">
              <Button
                variant="ghost"
                size="sm"
                className="text-pfw-gold hover:text-gold-dark hover:bg-pfw-gold/10">
                View All
              </Button>
            </Link>
          </div>
          ...
          {upcomingRsvps.length === 0 ? (
            <Card className="bg-white border-gray-200 pt-6">
              <CardContent className="py-16 text-center">
                ...
                <p className="text-gray-600 max-w-sm mx-auto mb-6">
                  ...
                  yet. Browse the catalog to find something
                  interesting!
                </p>
                <Link href="/events">
